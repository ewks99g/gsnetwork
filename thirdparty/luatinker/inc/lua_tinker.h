...
namespace lua_tinker
{
	...
	void	call_stack_info(lua_State* L, int n);

	// plus debug
	void	output_info(bool fend,const char* fmt, ...);
	void	init_break(lua_State *L);
	int		break_point(lua_State *L);	
	void	clear_func(lua_State *L, const char *funcname);

	int call_errorfunc(lua_State *L);

	// output
	typedef	void (OutCallBack)(bool, const char*);
	static	OutCallBack* G_OUTPUT;
	void	set_output(OutCallBack* pf);

	// functor fix
	typedef	void (FixCallBack)(const char*);
	static	FixCallBack* G_FUNCFIX;
	void	set_funcfix(FixCallBack* pf);

	// functor fix
	typedef	void (CallErrorBack)();
	static	CallErrorBack* G_CALLERROR;
	void	set_errorfunc(CallErrorBack* pf);
	...
	// functor
	template<typename RVal, typename T1=void, typename T2=void, typename T3=void, typename T4=void, typename T5=void, typename T6=void, typename T7=void, typename T8=void, typename T9=void>
	struct functor
	{
		static int invoke(lua_State *L) {
            push(L,upvalue_<RVal(*)(T1,T2,T3,T4,T5,T6,T7,T8,T9)>(L)(read<T1>(L,1),read<T2>(L,2),read<T3>(L,3),read<T4>(L,4),read<T5>(L,5),read<T6>(L,6),read<T7>(L,7),read<T8>(L,8),read<T9>(L,9))); return 1; }
	};
	...
	// member function
	template<typename RVal, typename T, typename T1=void, typename T2=void, typename T3=void, typename T4=void, typename T5=void,typename T6=void,typename T7=void,typename T8=void,typename T9=void>
	struct mem_functor
	{
	...
	void setsub(lua_State* L, const char* name, T object)
	{
		...
		if ((size_pos = strSrc.find_first_of('.', size_pos)) == std::string::npos)
		{
			lua_pushstring(L, name);
			push(L, object);
			lua_settable(L, LUA_GLOBALSINDEX);
		...
		lua_getfield(L, LUA_GLOBALSINDEX,strTemp.c_str());

		while((size_pos = strSrc.find_first_of('.', size_pos)) != std::string::npos)
		{
			strTemp = strSrc.substr(size_prev_pos, size_pos - size_prev_pos);

			size_prev_pos = ++size_pos;

			lua_getfield(L, -1,strTemp.c_str());
			lua_remove(L,-2);
		}

		strTemp = strSrc.substr(size_prev_pos);
		assert(false == strTemp.empty());			

		push(L, object);

		lua_setfield(L, -2,strTemp.c_str());
		
		//Edit By Wangbin,
		//C++和lua之间的栈一直在变大，printf("The elment of stack 1 is %d\n",lua_gettop(L));
		lua_pop(L, 1);
		lua_settop(L, 0);
	}
	...
	template<typename T>
	T getsub(lua_State* L, const char* name)
	{
		std::string strSrc(name);
...
//		lua_remove(L, -2);
	...
	struct table_obj
	{
		...
		template<typename T>
		void set(lua_Integer id, T object)
		{
			if(validate())
			{
