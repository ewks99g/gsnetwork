...
void lua_tinker::set_output(OutCallBack* pf)
{
	G_OUTPUT = pf;
}

void lua_tinker::set_funcfix(FixCallBack* pf)
{
	G_FUNCFIX = pf;
}
...
static int tostring_s64(lua_State *L)
{
	char temp[64];
#ifdef ANDROID	
    int value = *(long long*)lua_topointer(L, 1);
...
static int tostring_u64(lua_State *L)
{
	char temp[64];
#ifdef ANDROID	
    unsigned int _value = *(unsigned long long*)lua_topointer(L, 1);
	sprintf(temp, "%I64u", _value );
#else
...
bool lua_tinker::dofile(lua_State *L, const char *filename)
{
	...
	lua_getfield(L, LUA_GLOBALSINDEX, "newonerror");
...
static void call_stack(lua_State* L, int n)
{
    ...
    if(lua_getstack(L, n, &ar) == 1)
	{
		...
		if(ar.name)
		{
			lua_tinker::print_error(L, "%s%s() : %s:%d", indent, ar.name, ar.short_src, ar.currentline);
			
			if (lua_tinker::G_FUNCFIX)
			{
				lua_tinker::G_FUNCFIX(ar.name);
			}
		}
		else
		{
			lua_tinker::print_error(L, "%sunknown : %s:%d", indent,ar.short_src, ar.currentline);
		}
...
void lua_tinker::call_stack_info(lua_State* L, int n)
{
	lua_Debug ar;
	if(lua_getstack(L, n, &ar) == 1)
	{
		...
		{
			indent = "->";
			lua_tinker::output_info(false,"[LUA_DEBUG] |\t[[call stack]]\n");
		}
		else
		{
			indent = "  ";
		...
		if(ar.name)
			lua_tinker::output_info(false,"[LUA_DEBUG] |%s %s:%d : %s()\n", indent,ar.short_src ,ar.currentline, ar.name);
		else
			lua_tinker::output_info(false,"[LUA_DEBUG] |%s %s:%d : unkown\n", indent,ar.short_src,ar.currentline);
		...
		do 
		{
			...
			if (indent)
			{
				...
				{
				case LUA_TNIL:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s", lua_typename(L, lua_type(L, -1)),indent);
					break;
				case LUA_TBOOLEAN:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s	%s\n", lua_typename(L, lua_type(L, -1)),indent, lua_toboolean(L, -1)?"true":"false");
					break;
				case LUA_TLIGHTUSERDATA:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s	0x%08p\n", lua_typename(L, lua_type(L, -1)),indent, lua_topointer(L, -1));
					break;
				case LUA_TNUMBER:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s	%f\n", lua_typename(L, lua_type(L, -1)),indent, lua_tonumber(L, -1));
					break;
				case LUA_TSTRING:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s	%s\n", lua_typename(L, lua_type(L, -1)),indent, lua_tostring(L, -1));					
					break;
				case LUA_TTABLE:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s	0x%08p\n", lua_typename(L, lua_type(L, -1)),indent, lua_topointer(L, -1));
					break;
				...
				case LUA_TUSERDATA:
					...
					break;
				case LUA_TTHREAD:
					lua_tinker::output_info(false, "[LUA_DEBUG] |\t\t\t\t\t|-%s:%s\n", lua_typename(L, lua_type(L, -1)),indent);
					break;
				}
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
		output_info(false,"[LUA_ERROR] |  ");
		output_info(false,text);
		output_info(true,"\n");
...
/*---------------------------------------------------------------------------*/ 
void lua_tinker::output_info(bool fend,const char* fmt, ...)
{
	char text[4096];
...
void lua_tinker::init_break(lua_State *L)
{
...
int lua_tinker::call_errorfunc(lua_State *L)
{	
	if(G_CALLERROR)
	{
		G_CALLERROR();
...
void lua_tinker::clear_func(lua_State *L, const char *funcname)
{
	////lua_tocfunction
	...
	if(lua_isfunction(L,-1))
	{
		lua_pushstring(L, funcname);			/// ½«´íÎóº¯ÊýÌáµ½Õ»¶¥
		lua_pushcfunction(L, lua_tinker::call_errorfunc);	/// Ìæ»»º¯Êý
		lua_settable(L, LUA_GLOBALSINDEX);	/// ÉèÖÃÕ»
...
void	lua_tinker::set_errorfunc(CallErrorBack* pf)
{
	G_CALLERROR = pf;
}